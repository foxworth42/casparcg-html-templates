...
TemplateHandler.prototype.fillParameters = function(params = {}, hideParentOnEmpty = false) {
    var self = this;
    Object.keys(params).forEach(function(parameter) {
        var elements = document.querySelectorAll("[data-param='"+parameter+"']");
...
TemplateHandler.prototype._toggleDisplayClasses = function(paramValue, element, hideParentOnEmpty) {
    if (paramValue === "") {
        element.classList.add(this.config["emptyElementClass"]);
        if (hideParentOnEmpty === true) {
            element.parentElement.classList.add(this.config["emptyParentElementClass"]);
        }
    } else {
        element.classList.remove(this.config["emptyElementClass"]);
        if (hideParentOnEmpty === true) {
            element.parentElement.classList.remove(this.config["emptyParentElementClass"]);
        }
    }
}
